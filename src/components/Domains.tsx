...
import {
  X, Laptop, PenTool, Shield, Code, Globe, Palette, Users, Wrench,
} from "lucide-react";
  ...
  return (
    ...
    >
      <style>{`
        ...
        .domain-card-anim {
          animation: domainFadeUp 0.5s ease-out forwards;
        }
        .modal-overlay {
          animation: domainFadeUp 0.2s ease-out forwards;
        }
      `}</style>

      <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold uppercase text-[#0D0D0D] mb-10 text-center px-4">
        Our{" "}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 max-w-7xl px-4 sm:px-6 md:px-8">
        {DOMAINS.map((domain, index) => (
          <button
            ...
            className="domain-card-anim bg-[#FFF7D6] border-[2px] sm:border-[3px] rounded-xl sm:rounded-2xl
      ...
      {selectedDomain && (
        <div
          className="modal-overlay fixed inset-0 bg-[#0D0D0D]/60 backdrop-blur-sm flex items-center justify-center z-[1000] p-4"
          onClick={(e) => e.target === e.currentTarget && setSelectedDomain(null)}
        >
          <div className="relative bg-[#FFF7D6] border-[3px] rounded-3xl shadow-[10px_10px_0_#0D0D0D] w-full max-w-4xl p-6 sm:p-8 max-h-[85vh] overflow-y-auto">
            <button
            ...
            <div className="hidden sm:block">
              ...
              <h4 className="text-2xl text-black font-extrabold text-center mb-6">Core Team</h4>
              <div className="flex flex-wrap justify-center gap-6">
                {getCoreMembers(selectedDomain.id).length > 0 ? (
                  getCoreMembers(selectedDomain.id).map((member) => (
                    <div className="bg-[#FFEFB4] border-[2px] rounded-xl p-4 w-[160px] text-center shadow-[3px_3px_0_#0D0D0D] flex flex-col items-center">
  <div className="w-20 h-20 mx-auto mb-2 rounded-full overflow-hidden border-2 border-[#0D0D0D] flex-shrink-0">
    <Image src={member.imagePath || "/placeholder.svg"} alt={member.name} width={80} height={80} className="object-cover w-full h-full object-top" />
  </div>
  <p className="text-xs font-bold text-[#0D0D0D] leading-tight mt-1">{member.name}</p>
  <p className="text-[10px] text-[#F2A516] uppercase font-semibold leading-tight mt-1">{member.role}</p>
</div>
                  ))
                ) : (
                  <p className="text-[#333333] italic">Core team details coming soon...</p>
                )}
              </div>
            </div>
          </div>
